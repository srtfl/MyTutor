...
export default function ClassroomPage() {
  ...
  // SUPABASE REALTIME SYNC - Track presence only
  useEffect(() => {
  ...
  const handleCanvasChange = useCallback(() => {
    // Excalidraw iframe doesn't expose internal state
    // For canvas sync, use Excalidraw's built-in collaboration features
  }, []);

  const handleMount = useCallback((excalidrawAPI: any) => {
    // Excalidraw is loaded as iframe
  }, []);
      ...
      if (!localTracksRef.current.audioTrack) {
        ...
        if (agoraClientRef.current.connectionState === 'DISCONNECTED') {
          await agoraClientRef.current.join(
            '9bb4c1b7a3c14481ba51b83074108c83', 
            ROOM_ID, 
            '007eJxTYNhmZdAYb3ZNxnOlg7HL/pKl+7ISGk6eOP6T27PyoFp41UEFBsukJJNkwyTzRONkQxMTC8OkRFPDJAtjA3MTQwOLZAvjWyUKWQ2BjAyWZxkYGKEQxJdiSM/JT0rM0U3OSSwuLsrPz9UtTi0uzszP0zVkYAAAET8meA==', 
            null
          );
        }
  ...
  return (
    ...
    <div className="flex h-screen w-screen bg-zinc-950 text-white overflow-hidden">
      ...
      {/* SIDEBAR */}
      <aside className="w-[340px] border-r border-zinc-800 bg-zinc-900 flex flex-col justify-between p-4 z-20 shrink-0 relative shadow-2xl">
        ...
        <div className="pt-4 border-t border-zinc-800 space-y-3 mt-4 shrink-0">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button 
              onClick={toggleAudio} 
              className={`flex items-center justify-center gap-2 py-2 border rounded transition-colors text-xs font-medium ${
      ...
      <main className="flex-1 relative bg-zinc-100">
        ...
        {/* Question Panel */}
        {activeQuestion && (
          <div className="absolute top-6 left-6 z-20 w-full max-w-[420px] max-h-[80vh] overflow-y-auto bg-zinc-950/95 backdrop-blur-md border border-zinc-800 shadow-2xl rounded-xl p-5 animate-in slide-in-from-left-8 fade-in duration-300">
            <button 
              onClick={() => { 
                setActiveQuestion(null); 
                setShowSolution(false); 
