...
export default function ClassroomPage() {
    ...
    channel.on('broadcast', { event: 'canvas-update' }, (message) => {
      try {
        ...
        editorRef.current.store.mergeRemoteChanges(() => {
          try {
            if (payload.added) {
              const validAdded = Object.values(payload.added).filter((r: any) => r && r.id && isShareable(r.id));
              if (validAdded.length > 0) editorRef.current.store.put(validAdded);
            }
            if (payload.updated) {
              const validUpdated = Object.values(payload.updated).map((rp: any) => Array.isArray(rp) ? rp[1] : rp).filter((r: any) => r && r.id && isShareable(r.id));
              if (validUpdated.length > 0) editorRef.current.store.put(validUpdated);
            }
            if (payload.removed) {
              const validRemoved = Object.keys(payload.removed).filter(id => isShareable(id));
  ...
  return (
    // NATIVE FLEXBOX: Splits the screen cleanly into Sidebar (Left) and Canvas (Right)
